...
Modified on Fri Apr 10 2026:
    - cleaned active parsing helpers while preserving public function names
    - removed duplicated immobile-processing logic and centralised frame-gap interpolation
    - tightened empty-frame and malformed-line handling
...
def process_behavioural_data(
    ...
    if data['pulse_end_times']:
        data['pulse_end_times'] = correct_overflow(data['pulse_end_times'], 'pulse', start_time)
    if data['pulse_start_times']: # JINGYU 20260710
        data['pulse_start_times'] = correct_overflow(data['pulse_start_times'], 'pulse_start', start_time)
    if data['block_statements']:
        data['block_statements'] = correct_overflow(
            data['block_statements'], 'block_statement', start_time
            )
    ...
    return {
        ...
        'pulse_end_times': data['pulse_end_times'],
        'pulse_start_times': data['pulse_start_times'],
        'trial_statements': data['trial_statements'],
        'new_trial_statements': data['new_trial_statements'],
        'block_statements': data['block_statements'], # Jingyu, 4/29/2026
...
def process_txt(txtfile):
    ...
    pulse_command_list = []
    block_statements = []  # block start markers; first $BT often precedes the first $TR

    line = ['']
    while line[0] != '$TR':
        if line[0] == '$PP':  # sometimes this is before the first trial
            pulse_command_list.append(line)
        if line[0] == '$BT':  # block-start marker for the first block usually arrives here
    ...
    while line[0].find('$') == 0:
        ...
        if line[0] == '$PC':
            pulse_end_times.append(float(line[1]))
        if line[0] == '$PO':
            pulse_start_times.append(float(line[1]))
        if line[0] == '$PP':
    ...
    curr_logfile['pulse_end_times'] = pulse_end_times
    curr_logfile['pulse_start_times'] = pulse_start_times
    ...
    curr_logfile['block_statements'] = block_statements
    return curr_logfile
    ...
    if label == 'lick':  # added by Jingyu 6/22/2024
        ...
        for t in range(tot_trial):
            if len(data[t])==0:  # if there is no lick, append an empty list
                ...
                new_data.append(new_trial)
    if label=='pulse' or label=='pulse_start': # Jingyu, 20260710
        try:
