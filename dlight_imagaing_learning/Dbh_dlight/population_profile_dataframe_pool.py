...
for k in DILATION_STEPS:
    ...
    for rec in rec_lst:
        ...
        p_regression = (OUR_DIR_REGRESS / rec / regression_name 
                        / f'dilation_k={k}')
        p_stats = OUT_DIR_DF / f'{rec}_profile_dilation={k}_stat_dlight_pre{dlight_pre}_post{dlight_post}_run_align.parquet'
        p_stats_red = OUT_DIR_DF / f'{rec}_profile_dilation={k}_stat_red_pre{dlight_pre}_post{dlight_post}_run_align.parquet'
        ...
        p_df_out = OUT_DIR_DF / rf"{rec}_profile_combined_dilation={k}_pre{dlight_pre}_post{dlight_post}_ES={effect_size_thresh}_shuff{amp_shuff_thresh_up}.parquet"
