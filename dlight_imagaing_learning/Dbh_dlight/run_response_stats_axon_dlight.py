...
for rec in tqdm(rec_lst):
    ...
    for k_size in tqdm(DILATION_STEPS):
        ...
        out_dir = OUT_DIR_RAW_DATA / 'processed_dataframe_grid_free_dilation'
        ...
        df_roi_stats.to_parquet(out_dir / f'{rec}_profile_dilation={k_size}_stat_dlight_pre{dlight_pre}_post{dlight_post}_run_align.parquet' )
        df_roi_stats_red.to_parquet(out_dir / f'{rec}_profile_dilation={k_size}_stat_red_pre{dlight_pre}_post{dlight_post}_run_align.parquet')
    
        df_roi_stats = quantify_event_response(corrected_traces = dff_dlight_sm, 
                                            event_frames=reward_frames_valid,
                                            baseline_window=dlight_pre, 
                                            response_window=dlight_post, # seconds
                                            dilation_k = k_size,
                                            imaging_rate=30.0, shuffle_test=True,
        ...
        df_roi_stats.to_parquet(out_dir / f'{rec}_profile_dilation={k_size}_stat_dlight_pre{dlight_pre}_post{dlight_post}_rew_align.parquet' )
        df_roi_stats_red.to_parquet(out_dir / f'{rec}_profile_dilation={k_size}_stat_red_pre{dlight_pre}_post{dlight_post}_rew_align.parquet')
