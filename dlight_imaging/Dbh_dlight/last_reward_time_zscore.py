...
for rec in tqdm(rec_lst):
    ...
    H, W, T = corrected_dlight.shape
    corrected_dlight_flat = corrected_dlight.reshape(H * W, T)
    dlight_zscore_short, _ = calculate_zscore_f_trace(corrected_dlight_flat, event_frames=run_onset_frames[short_trials_indx],)
    dlight_zscore_long, _ =  calculate_zscore_f_trace(corrected_dlight_flat, event_frames=run_onset_frames[long_trials_indx],)
    dlight_zscore_short = dlight_zscore_short.reshape(H, W, T)
    dlight_zscore_long  = dlight_zscore_long.reshape(H, W, T)
    # all rois
    ...
    dlight_zscore_up_short = dlight_zscore_short[y, x, :]
    dlight_zscore_up_long = dlight_zscore_long[y, x, :]
    
    dff_dlight_safe = np.apply_along_axis(trace_filter, axis=-1, arr=dlight_dff_up, n_sd=5)
    dff_dlight_sm = cp_gaussian_filter1d(cp.array(dff_dlight_safe), 
                                               sigma=1).get()
    dlight_zscore_short_sm = cp_gaussian_filter1d(cp.array(dlight_zscore_up_short), 
                                               sigma=1).get()
    dlight_zscore_long_sm = cp_gaussian_filter1d(cp.array(dlight_zscore_up_long), 
                                               sigma=1).get()
    dlight_dff_aligned = align_trials(dff_dlight_sm, alignment='run', beh=beh,
                                          bef=bef, aft=aft)
    dlight_zscore_aligned_short = align_trials(dlight_zscore_short_sm, alignment='run', beh=beh,
                                          bef=bef, aft=aft)
    dlight_zscore_aligned_long = align_trials(dlight_zscore_long_sm, alignment='run', beh=beh,
                                          bef=bef, aft=aft)
    ...
    short_trials_mean_zscore = np.nanmean(dlight_zscore_aligned_short[:, short_trials_indx, :], axis=1)
    long_trials_mean_zscore = np.nanmean(dlight_zscore_aligned_long[:, long_trials_indx, :], axis=1)
