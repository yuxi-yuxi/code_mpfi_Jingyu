...
for rec_id, df_session in df_pool_sorted.groupby('rec_id'):
    OUT_DIR_FIG = Path(rf"Z:\Jingyu\LC_HPC_manuscript\raw_data\geco_dlight\TEST_PLOTS\singl_session_plot\{rec_id}")
    ...
    df_pool_dlight_up = df_session.loc[df_session['dlightUp']]
    df_pool_non_dlight_up = df_session.loc[~df_session['dlightUp']]
    ...
    try:
        ...
        df_session = df_session.sort_values(by=['dlight_type', 'effect_size'], ascending=[False, False])
        ...
        roi_types = df_session['dlight_type'].values
        ...
        # all soma rois GECO
        df_session = df_session.sort_values(by=['geco_type', key_geco_ratio], ascending=[False, False])
        # get the 'geco_mean_trace' column in that sorted order
        traces = np.stack(df_session[key_geco_profile])
        traces = gaussian_filter1d(traces, sigma=1)
        traces = normalize(traces)
        fig, ax = plt.subplots(figsize=(3,3), dpi=300)
        ax.imshow(traces,
        ...
        roi_types = df_session['geco_type'].values
        change_idx = np.where(roi_types[:-1] != roi_types[1:])[0] + 1  # row indices where type changes
        n = traces.shape[0]
        change_idx_plot = n - change_idx
        for y in change_idx_plot:
        ...
        dlightUp_pyrUp_geco_traces         = factor*np.stack(df_session.loc[(df_session['dlight_type']=='Up')&
                                                          (df_session['geco_type']=='Up'), key_geco_profile])
        dlightUp_pyrDown_geco_traces       = factor*np.stack(df_session.loc[(df_session['dlight_type']=='Up')&
                                                          (df_session['geco_type']=='Down'), key_geco_profile])
        
        
        non_dlightUp_pyrUp_geco_traces     = factor*np.stack(df_session.loc[(df_session['dlight_type']!='Up')&
                                                              (df_session['geco_type']=='Up'), key_geco_profile])
        
        non_dlightUp_pyrDown_geco_traces   = factor*np.stack(df_session.loc[(df_session['dlight_type']!='Up')&
                                                              (df_session['geco_type']=='Down'), key_geco_profile])
        
        factor=100
        dlightUp_pyrUp_dlight_traces       = factor*np.stack(df_session.loc[(df_session['dlight_type']=='Up')&
                                                             (df_session['geco_type']=='Up'), 'mean_profile'])
        
        dlightUp_pyrDown_dlight_traces     = factor*np.stack(df_session.loc[(df_session['dlight_type']=='Up')&
                                                             (df_session['geco_type']=='Down'), 'mean_profile'])
        non_dlightUp_pyrUp_dlight_traces   = factor*np.stack(df_session.loc[(df_session['dlight_type']!='Up')&
                                                                 (df_session['geco_type']=='Up'), 'mean_profile'])
        non_dlightUp_pyrDown_dlight_traces = factor*np.stack(df_session.loc[(df_session['dlight_type']!='Up')&
                                                                 (df_session['geco_type']=='Down'), 'mean_profile'])
            
        dlightUp_dlight_traces      = factor*np.stack(df_session.loc[(df_session['dlight_type']=='Up'), 'mean_profile'])
        non_dlightUp_dlight_traces  = factor*np.stack(df_session.loc[~(df_session['dlight_type']=='Up'), 'mean_profile'])
        ...
        df_perc = calculate_percs(df_session)
        
        # Calculate percentages per recording
        df_perc_pool = df_session.groupby('rec_id').apply(calculate_percs).apply(pd.Series)
