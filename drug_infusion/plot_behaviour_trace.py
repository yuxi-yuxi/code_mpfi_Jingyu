...
from scipy.stats import wilcoxon, iqr, binned_statistic
...
def extract_behaviour_info(rec_lst, PATH_beh, labels):
    df_beh_pool = pd.DataFrame(
        {'anm_id': str,
         ...
         'lick_dist_norm_mean': [],                # normalised by session peak (over distance)
         'lick_time_profile': [], 
    ...
    for i, rec in rec_lst.iterrows():
        ...
        for i, ss in enumerate(['02', '04']):
            ...
            licks_dist_map = np.vstack([licks_dist_map[t] for t in range(n_trials) if valid_trials[t]])
            ...
            lick_dist_norm_mean = np.nanmean(lick_dist_norm, axis=0)
            ...
            speed_time_aligned_array = np.vstack([zero_padding(speed_time_aligned[t], 4000) 
                                                  for t in range(n_trials) if valid_trials[t]])

            speed_time_aligned_mean = np.nanmean(speed_time_aligned_array, axis=0)
            
            speed_dist = beh['speed_distances_aligned']
            speed_dist = np.vstack([speed_dist[t] for t in range(n_trials) if valid_trials[t]])
...
out_dir = r"Z:\Jingyu\LC_HPC_manuscript\fig_infusion\behaviour_0708"
...
# by distance
licks_profile_drug = df_beh_drug.loc[df_beh_drug['label']==drug, 'lick_distance_profile_mean']
licks_profile_drug = np.vstack(licks_profile_drug)
licks_profile_ctrl = df_beh_ctrl.loc[df_beh_ctrl['label']=='ctrl', 'lick_distance_profile_mean']
...
fig, ax = pf.plot_two_traces_with_binned_stats(licks_profile_drug , licks_profile_ctrl,
                                  colors=['orange', 'grey'],
                                  labels=[f'{drug}', 'ctrl'],
                                  # time_windows=[(0, 45), (45, 90), (90, 135), (135, 180)],
                                  time_windows=[(30, 100), (120, 180)],
                                  baseline_window=None,
                                  bef=0, aft=220, sample_freq=1,
                                  )
ax.set(xlabel='Distance (cm)', ylabel='Licks/cm',
       ylim=(0, 0.15), xlim=(30, 220)
       )
plt.tight_layout()
for form in ['pdf', 'png']:
    plt.savefig(out_dir+r'\lick_trace_dist_comparision_{}.{}'.format(drug, form),
            dpi=300)
plt.show()

# by distance norm
# licks_profile_drug_norm = licks_profile_drug/np.nanmax(licks_profile_drug, axis=1)[:, None]
# licks_profile_ctrl_norm = licks_profile_ctrl/np.nanmax(licks_profile_ctrl, axis=1)[:, None]

licks_profile_drug_norm = licks_profile_drug/np.nanmax(licks_profile_drug)
licks_profile_ctrl_norm = licks_profile_ctrl/np.nanmax(licks_profile_ctrl)

fig, ax = pf.plot_two_traces_with_binned_stats(licks_profile_drug_norm, licks_profile_ctrl_norm,
                                  colors=['orange', 'grey'],
                                  labels=[f'{drug}', 'ctrl'],
                                  # time_windows=[(0, 45), (45, 90), (90, 135), (135, 180)],
                                  time_windows=[(30, 100), (120, 180)],
                                  baseline_window=None,
                                  bef=0, aft=220, sample_freq=1,
                                  print_median_iqr=True
                                  
                                  )
ax.set(xlabel='Distance (cm)', ylabel='Licks norm',
       ylim=(0, 1), xlim=(30, 220)
       )
plt.tight_layout()
for form in ['pdf', 'png']:
    plt.savefig(out_dir+r'\lick_trace_dist_norm_comparision_{}.{}'.format(drug, form),
            dpi=300)
plt.show()
...
ax.set(xlabel='time from run (s)', ylabel='Licks (Hz)',
       ylim=(0, 25), xlim=(0, 4)
       )
plt.tight_layout()
for form in ['pdf', 'png']:
    plt.savefig(out_dir+r'\lick_trace_time_comparision_{}.{}'.format(drug, form),
            dpi=300)
plt.show()  
...
#%% mean speed trace
# by distance
speeds_profile_drug = df_beh_drug.loc[df_beh_drug['label']==drug, 'speed_distance_profile_mean']
speeds_profile_drug = np.vstack(speeds_profile_drug)
speeds_profile_ctrl = df_beh_ctrl.loc[df_beh_ctrl['label']=='ctrl', 'speed_distance_profile_mean']
...
fig, ax = pf.plot_two_traces_with_binned_stats(speeds_profile_drug, speeds_profile_ctrl,
                                  ...
                                  time_windows=[(0, 180),],
                                  baseline_window=None,
                                  bef=0, aft=220, sample_freq=10,
                                  print_median_iqr=True
                                  )
ax.set(xlabel='Distance (cm)', ylabel='Speed (cm/s)',
       xlim=(0, 180), ylim=(0,70)

       )
plt.tight_layout()
for form in ['pdf', 'png']:
    plt.savefig(out_dir+r'\speed_trace_dist_comparision_{}.{}'.format(drug, form),
            dpi=300) 
plt.show() 
...
plt.tight_layout()
for form in ['pdf', 'png']:
    plt.savefig(out_dir+r'\speed_trace_time_comparision_{}.{}'.format(drug, form),
            dpi=300) 
plt.show() 
...
rew_rate_drug = df_beh_drug.loc[df_beh_drug['label']==drug, 'rew_rate']*100
rew_rate_ctrl = df_beh_ctrl.loc[df_beh_ctrl['label']=='ctrl', 'rew_rate']*100

fig, ax = plt.subplots(figsize=(2, 3), dpi=300)
pf.plot_bar_with_unpaired_scatter(ax, rew_rate_drug, rew_rate_ctrl,
                                  xticklabels=[f'{drug}', 'saline'],
                                  colors=['orange', 'grey'],
                                  jitter=0.01, ylabel='Reward rate (%)',
                                  print_median_iqr=True)
ax.set(ylim=(0,100))
plt.tight_layout()
for form in ['pdf', 'png']:
    plt.savefig(out_dir+r'\rew_rate_comparision_{}.{}'.format(drug, form),
            dpi=300) 
plt.show()
#%% speed mean 
# fl_time_drug = df_beh_drug.loc[df_beh_drug['label']==drug, 'first_licks_time_median']
