...
<body>
    ...
    <script>
        ...
        function updateTrace(cellId, session) {{
            ...
            // Compute cell type dynamically from current thresholds
            let cellType = 'Unknown';
            if (info && info.info_bits !== null) {{
                ...
                if (currentMeanDffThresh > 0) {{
                    if (info.peak_amplitude === null || info.peak_amplitude <= currentMeanDffThresh) {{
                        isSig = false;
                    }}
                }}
                if (currentPercActiveLapsThresh > 0) {{
                    if (info.perc_active_laps === null || info.perc_active_laps <= currentPercActiveLapsThresh) {{
                        isSig = false;
                    }}
                }}
                if (currentPercActiveFramesThresh > 0) {{
                    if (info.perc_active_frames === null || info.perc_active_frames <= currentPercActiveFramesThresh) {{
                        isSig = false;
                    }}
                }}
                cellType = isSig ? cellTypeLabel : nonCellTypeLabel;
            }}
