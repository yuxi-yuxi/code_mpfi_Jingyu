...
import base64
import io
import place_cell_functions as pcf
...
# select_significant_cells lives in place_cell_functions.py — call via pcf.select_significant_cells

# --- Old threshold-based selection (commented out) ---
# def select_significant_cells_old(df, config,
#                              info_threshold,
#                              shuff_thresh,
...
#             if arr.size == 0 or np.all(np.isnan(arr)): return np.nan
...
# --- old wrappers (commented out — signature changed) ---
...
def _build_mode_data(df_valid, xaxis, config, roi_stat, active_soma_indices):
    ...
    # peak_amplitude_col = config['peak_amplitude_col']  # commented out — not used with field-based selection
    # shuffled_col = config.get('shuffled_info_col')  # commented out
    ...
    for _, row in df_valid.iterrows():
        ...
        # Pre-computed per-field metrics (one value per tentative field)
        def _to_float_list(col_name):
        ...
        field_width_cm = _to_float_list('tentative_field_width_cm')
        field_peak_dff = _to_float_list('tentative_field_peak_dff')
        field_in_out_ratio = _to_float_list('tentative_field_in_out_ratio')
        field_transient_frac = _to_float_list('tentative_field_transient_fraction')
        ...
        cell_info[key] = {
            ...
            'field_peak_dff': field_peak_dff,
            'field_in_out_ratio': field_in_out_ratio,
            'field_transient_frac': field_transient_frac,
        }
...
def load_roi_stat(rec_id, config, rec_name=None):
    if rec_name is None:
        parts = rec_id.split('-')
        ...
        gcamp_stats_path = Path(config['gcamp_stats_pattern'].format(anm=anm, date=date, ss=ss))
        if not gcamp_stats_path.exists():
            raise FileNotFoundError(f"Could not find gcamp_stats.npy for {rec_id}: {gcamp_stats_path}")
        gcamp_stats = np.load(gcamp_stats_path, allow_pickle=True)
    
        soma_class_path = Path(config['soma_class_pattern'].format(anm=anm, date=date, ss=ss))
        if not soma_class_path.exists():
            raise FileNotFoundError(f"Could not find soma_class.npz for {rec_id}: {soma_class_path}")
        soma_data = np.load(soma_class_path)
        is_active_soma = soma_data['is_soma']
...
def _load_mode_for_both_sessions(rec_ss1, rec_ss2, config, xaxis,
                                  ...
                                  min_peak_dff=0.1, min_in_out_ratio=2.0,
                                  min_width_cm=12.0, min_transient_fraction=0.15):
    ...
    df1 = pcf.select_significant_cells(df1, config, min_peak_dff, min_in_out_ratio, min_width_cm, min_transient_fraction)
    df2 = pcf.select_significant_cells(df2, config, min_peak_dff, min_in_out_ratio, min_width_cm, min_transient_fraction)
    ...
    bundle_ss1 = _build_mode_data(df1_shared, xaxis, config, roi_stat, active_soma_indices)
    bundle_ss2 = _build_mode_data(df2_shared, xaxis, config, roi_stat, active_soma_indices)
    return {'ss1': bundle_ss1, 'ss2': bundle_ss2}


def _load_mode_for_single_session(rec, config, xaxis,
                                   roi_stat, active_soma_indices,
                                   min_peak_dff=0.1, min_in_out_ratio=2.0,
                                   min_width_cm=12.0, min_transient_fraction=0.15):
    """Load df for one session of a mode and build the bundle."""
    try:
        df = load_dataframe(rec, config)
    except FileNotFoundError as e:
        print(f"  [{config['cell_type_label']}] missing: {e}")
        return None

    df = pcf.select_significant_cells(df, config, min_peak_dff, min_in_out_ratio,
                                      min_width_cm, min_transient_fraction)
    print(f"  [{config['cell_type_label']}] cells: {len(df)}")
    if len(df) == 0:
        return None
    return _build_mode_data(df, xaxis, config, roi_stat, active_soma_indices)


def process_behaviour_data_single(behaviour):
    """Process a single behaviour pickle into a JSON-ready dict."""
    if behaviour is None:
        return None
    ss_result = {}
    try:
        lick_selectivities = behaviour.get('lick_selectivities', None)
        ...
        total_ms = 4000
        speed_trials = []
        for tr_speed in behaviour.get('speed_times_aligned', []):
            if len(tr_speed) > 0:
        ...
        lick_times_all = []
        for licks in behaviour.get('lick_times_aligned', []):
        ...
        if len(lick_times_all) > 0:
            ...
            n_trials = len(behaviour.get('speed_times_aligned', []))
            if n_trials > 0:
    ...
    return ss_result if ss_result else None
...
<body>
    ...
    <div class="controls">
        ...
        <span>
            ...
            <input type="number" id="threshold-input" step="0.05" min="0" value="0.2">
        </span>
        <span>
            <label id="shuff-thresh-label" for="shuff-thresh-input">Shuffle Percentile:</label>
            <input type="number" id="shuff-thresh-input" step="1" min="0" max="100" value="99">
        </span>
        <span>
            <label for="mean-dff-thresh-input">Mean dF/F Threshold:</label>
            <input type="number" id="mean-dff-thresh-input" step="0.01" min="0" value="0.1">
        </span>
        <span>
            <label for="perc-active-laps-thresh-input">% Active Laps Threshold:</label>
            <input type="number" id="perc-active-laps-thresh-input" step="0.01" min="0" max="1" value="0.1">
        </span>
        <span>
            <label for="perc-active-frames-thresh-input">% Active Frames Threshold:</label>
            <input type="number" id="perc-active-frames-thresh-input" step="0.01" min="0" max="1" value="0.1">
        </span>
    ...
    <script>
        ...
        // --- old threshold vars (commented out) ---
        ...
        let signalLabel, xLabel, infoLabel, cellTypeLabel, nonCellTypeLabel;
        let xaxis, maxXLimit;
        let vmin, vmax;
        let fovRoiData;
        ...
        // Check if cell has at least one tentative field passing all 4 Dombeck criteria
        function hasAcceptedField(info) {{
            if (!info.field_width_cm || info.field_width_cm.length === 0) return false;
            const n = info.field_width_cm.length;
            for (let i = 0; i < n; i++) {{
                if (info.field_width_cm[i] >= currentMinWidthCm &&
                    info.field_peak_dff[i] >= currentMinPeakDff &&
                    info.field_in_out_ratio[i] >= currentMinInOutRatio &&
                    info.field_transient_frac[i] >= currentMinTransientFrac) {{
                    return true;
                }}
            }}
            return false;
        ...
        // Check if a single field passes (for per-field display)
        function isFieldAccepted(info, idx) {{
            return info.field_width_cm[idx] >= currentMinWidthCm &&
                   info.field_peak_dff[idx] >= currentMinPeakDff &&
                   info.field_in_out_ratio[idx] >= currentMinInOutRatio &&
                   info.field_transient_frac[idx] >= currentMinTransientFrac;
        }}

        function isCellSignificant(info) {{
            if (!info) return false;
            return hasAcceptedField(info);
        }}
        ...
        function applyModePointers(mode) {{
            ...
            // --- old threshold pointers (commented out) ---
        ...
        function updateTrace(cellId, session) {{
            ...
            let fieldsHtml = '';
            if (info && info.field_width_cm && info.field_width_cm.length > 0) {{
                const nFields = info.field_width_cm.length;
                const nAccepted = Array.from({{length: nFields}}, (_, i) => isFieldAccepted(info, i)).filter(Boolean).length;
                fieldsHtml = `<br>Fields: ${{nAccepted}}/${{nFields}} accepted`;
                for (let i = 0; i < nFields; i++) {{
                    const ok = isFieldAccepted(info, i);
                    const color = ok ? 'green' : '#999';
                    const tag = ok ? '&#10003;' : '&#10007;';
                    fieldsHtml += `<br><span style="color:${{color}}">  ${{tag}} F${{i+1}}: ` +
                        `w=${{info.field_width_cm[i].toFixed(0)}}cm, ` +
                        `peak=${{info.field_peak_dff[i].toFixed(3)}}, ` +
                        `in/out=${{info.field_in_out_ratio[i].toFixed(1)}}, ` +
                        `tf=${{info.field_transient_frac[i].toFixed(2)}}</span>`;
                }}
            }} else {{
                fieldsHtml = '<br>No tentative fields';
            }}
            document.getElementById(`roi-info-${{session}}`).innerHTML =
                ...
                `Peak pos: ${{peakPosStr}}` + fieldsHtml;
        ...
        function recomputeSignificance(session) {{
            const st = state[session];
            if (!st) return;
            const sigIds = [];
            const sigPeaks = [];
        ...
        function updateThresholdUI() {{
            // --- old threshold UI sync (commented out) ---
        ...
        // --- old threshold listeners (commented out) ---
        // document.getElementById('threshold-input').addEventListener('input', ...);
        // document.getElementById('shuff-thresh-input').addEventListener('input', ...);
        // document.getElementById('mean-dff-thresh-input').addEventListener('input', ...);
        // document.getElementById('perc-active-laps-thresh-input').addEventListener('input', ...);
        // document.getElementById('perc-active-frames-thresh-input').addEventListener('input', ...);

        // Dombeck field criteria inputs
        function recomputeAll() {{
            for (const s of SESSIONS) recomputeSignificance(s);
            syncSigStabilityYRange();
        }}
...
def _single_session_panels_html():
    """Panels block for the single-session HTML (no per-session label / stats prefix)."""
    return '''
        <div id="session-col-main" class="session-row">
            <div class="session-header">
                <div class="stats" id="stats-counter-main">Significant: -/-</div>
            </div>
            <div id="roi-info-main" class="info">Click an ROI on the map to view its field map.</div>
            <div class="session-panels">
                <div id="trace-panel-main" class="panel">
                    <div id="trace-plot-main"></div>
                </div>
                <div id="per-lap-panel-main" class="panel">
                    <div id="per-lap-heatmap-main"></div>
                </div>
                <div id="sequence-panel-main" class="panel">
                    <div id="sequence-heatmap-main"></div>
                </div>
                <div id="stability-panel-main" class="panel">
                    <div id="stability-hist-main"></div>
                </div>
                <div id="sig-stability-panel-main" class="panel">
                    <div id="sig-stability-box-main"></div>
                </div>
            </div>
        </div>
    '''
...
def generate_interactive_html_single_session(rec_id, mean_img,
                                              time_data=None, place_data=None,
                                              save_path=None,
                                              behaviour_data=None):
    """
    Generate interactive HTML for one session (single FOV).

    time_data / place_data : dict, optional
        A single bundle as produced by `_build_mode_data` / `_load_mode_for_single_session`.
        At least one of the two must be supplied.

    behaviour_data : dict, optional
        The raw behaviour pickle dict for this session (will be processed internally).
    """
    if time_data is None and place_data is None:
        raise ValueError("Must provide at least one of time_data or place_data")
    ...
    mode_data_js = {}
    if has_time:
        mode_data_js['time'] = {'main': time_data}
    if has_place:
        mode_data_js['place'] = {'main': place_data}
    mode_data_json = json.dumps(_round_for_json(mode_data_js))

    beh_dict = process_behaviour_data_single(behaviour_data)
    beh_data_json = json.dumps({'main': beh_dict})
    ...
    session_panels_html = _single_session_panels_html()

    init_ref = mode_data_js[initial_mode]['main']
    init_min_peak_dff = init_ref['minPeakDff']
    init_min_in_out_ratio = init_ref['minInOutRatio']
    init_min_width_cm = init_ref['minWidthCm']
    init_min_transient_frac = init_ref['minTransientFraction']
...
<head>
    <meta charset="UTF-8">
    <title>{rec_id} - Interactive Time/Place Cell Map</title>
    <script src="https://cdn.plot.ly/plotly-2.27.0.min.js"></script>
    <style>
        body {{ font-family: Arial, sans-serif; margin: 20px; background-color: #f5f5f5; }}
        .container {{ display: flex; gap: 20px; flex-wrap: wrap; align-items: stretch; }}
        ...
        .session-header {{ display: flex; align-items: baseline; gap: 15px; }}
        .session-panels {{ display: flex; flex-direction: row; flex-wrap: wrap; gap: 12px; align-items: flex-start; }}
        .session-panels > .panel {{ flex: 0 0 auto; box-sizing: border-box; }}
        .session-panels [id^="trace-plot-"] {{ width: 15vw; height: 30vh; }}
        .session-panels [id^="per-lap-heatmap-"],
        .session-panels [id^="sequence-heatmap-"] {{ width: 13vw; height: 30vh; }}
        .session-panels [id^="stability-hist-"],
        .session-panels [id^="sig-stability-box-"] {{ width: 11vw; height: 30vh; }}
        .legend {{ display: flex; gap: 15px; margin-top: 10px; font-size: 12px; }}
        .legend-item {{ display: flex; align-items: center; gap: 5px; }}
        .legend-box {{ width: 15px; height: 15px; }}
        .controls {{
...
<body>
    ...
    <script>
        ...
        function createBehaviourPlot() {{
            const bd = behData[SESSION_ID];
            const traces = [];
            if (bd) {{
                const lineColor = 'steelblue';
                const fillColor = 'rgba(70,130,180,0.4)';
                if (bd.speedMean && bd.speedXaxis) {{
                    const upper = bd.speedMean.map((m, i) => m + (bd.speedSem ? bd.speedSem[i] : 0));
                    const lower = bd.speedMean.map((m, i) => m - (bd.speedSem ? bd.speedSem[i] : 0));
                    traces.push({{
                    ...
                    traces.push({{
                        x: bd.speedXaxis, y: bd.speedMean, mode: 'lines',
                        name: 'Speed', line: {{ color: lineColor, width: 1.5 }},
                        yaxis: 'y'
                    }});
                }}
                if (bd.lickRate && bd.lickXaxis) {{
                    traces.push({{
                        x: bd.lickXaxis, y: bd.lickRate, type: 'bar',
                        name: 'Lick rate', marker: {{ color: fillColor }},
                        width: 0.08, yaxis: 'y2'
                    }});
                }}
            }}
            const titleSuffix = (bd && bd.lickIndex != null) ? ` | LI: ${{bd.lickIndex.toFixed(3)}}` : '';
            const layout = {{
                title: `Behaviour${{titleSuffix}}`,
                autosize: true,
                xaxis: {{ title: 'Time (s)', range: [0, 4] }},
        ...
        function applyModePointers(mode) {{
            ...
            vmin    = ref.vmin;
            vmax    = ref.vmax;
            fovRoiData = ref.roiData;

            state = {{
                cellInfo: ref.cellInfo,
                traces: ref.traces,
                roiData: ref.roiData,
                perLapData: ref.perLapData,
                sequenceHeatmap: ref.sequenceHeatmap,
                sequenceCellIds: ref.sequenceCellIds,
                sequencePeakTimes: ref.sequencePeakTimes,
                stabilityValues: ref.stabilityValues,
                stabilityMedian: ref.stabilityMedian,
                stabilityMean: ref.stabilityMean,
                sigStabilityValues: ref.sigStabilityValues,
                sigStabilityMedian: ref.sigStabilityMedian,
                sigStabilityMean: ref.sigStabilityMean,
                totalCells: Object.keys(ref.cellInfo).length,
            }};
        }}
        ...
        function createResponseMap() {{
            ...
            const colorSrcData = state.roiData;
            ...
            const roiOverlay = {{
                z: roiMaskMap, type: 'heatmap', colorscale: 'Viridis',
                zmin: vmin, zmax: vmax, hoverongaps: false,
        ...
        function updateTrace(cellId) {{
            if (!state) return;
            const key = cellId.toString();
            const perLap = state.perLapData[key];
            const trace = state.traces[key];
            const info = state.cellInfo[key];
            const cellType = isCellSignificant(info) ? cellTypeLabel : nonCellTypeLabel;
            ...
            plot('trace-plot-main', plotData.length > 0 ? plotData : [], layout);
            ...
            document.getElementById('roi-info-main').innerHTML =
                `<strong>Cell ${{cellId}}</strong> | ` +
                `Type: <span style="color: ${{cellType === cellTypeLabel ? 'green' : 'blue'}}">${{cellType}}</span> | ` +
                `Peak pos: ${{peakPosStr}}` + fieldsHtml;
        ...
        function updatePerLapHeatmap(cellId) {{
            ...
            const perLap = state.perLapData[key];
            if (!perLap || perLap.length === 0) {{
                plot('per-lap-heatmap-main', [], {{
                    title: `Cell ${{cellId}} - Per-Lap (No data)`,
                    autosize: true,
                    margin: {{ t: 40, b: 60, l: 60, r: 80 }}
                }});
                return;
            ...
            const layout = {{
                title: `Cell ${{cellId}} - Per-Lap (${{nTrials}} trials)`,
                autosize: true,
                xaxis: {{ title: xLabel, range: [0, maxXLimit] }},
                yaxis: {{ title: 'Trial', autorange: 'reversed' }},
                margin: {{ t: 40, b: 100, l: 60, r: 30 }}
            }};
            plot('per-lap-heatmap-main', [heatmapTrace], layout);
        }}

        function createSequenceHeatmap() {{
            if (!state || !state.sequenceHeatmap || state.sequenceHeatmap.length === 0) {{
                plot('sequence-heatmap-main', [], {{
                    title: `${{cellTypeLabel}} Sequence (None)`,
                    autosize: true,
                    margin: {{ t: 40, b: 60, l: 60, r: 80 }}
                }});
                return;
            }}
            const nCells = state.sequenceHeatmap.length;
            const heatmapTrace = {{
                z: state.sequenceHeatmap,
                x: xaxis,
                y: Array.from({{ length: nCells }}, (_, i) => i),
                type: 'heatmap', colorscale: 'Greys',
                colorbar: {{
                ...
                hovertemplate: 'Cell %{{customdata}}<br>X: %{{x:.2f}}<br>Activity: %{{z:.3f}}<extra></extra>',
                customdata: state.sequenceCellIds.map(id => Array(xaxis.length).fill(id))
            }};
            const layout = {{
                title: `${{cellTypeLabel}}s (${{nCells}})`,
                autosize: true,
                xaxis: {{ title: xLabel, range: [0, maxXLimit] }},
                yaxis: {{ title: 'Cells', tickmode: 'array', tickvals: [], ticktext: [], autorange: 'reversed' }},
                margin: {{ t: 40, b: 100, l: 60, r: 30 }}
            }};
            plot('sequence-heatmap-main', [heatmapTrace], layout);
            document.getElementById('sequence-heatmap-main').on('plotly_click', function(data) {{
                const p = data.points[0];
                const cellIdx = Math.round(p.y);
                if (cellIdx >= 0 && cellIdx < state.sequenceCellIds.length) {{
                    const cellId = state.sequenceCellIds[cellIdx];
                    selectRoi(cellId.toString());
                }}
            }});
        }}
        ...
        function highlightSequenceCell(roiId) {{
            if (!state) return;
            const div = document.getElementById('sequence-heatmap-main');
            if (!div || !state.sequenceHeatmap || state.sequenceHeatmap.length === 0) return;
            const cellId = parseInt(roiId);
            const cellIdx = state.sequenceCellIds.indexOf(cellId);
            let shapes = [];
            if (cellIdx >= 0) {{
                shapes.push({{
                    type: 'rect',
            ...
            Plotly.relayout('sequence-heatmap-main', {{ shapes: shapes }});
        }}

        function createStabilityHistogram() {{
            if (!state || !state.stabilityValues || state.stabilityValues.length === 0) {{
                plot('stability-hist-main', [], {{
                    title: `Stability (No data)`,
                    autosize: true,
                    margin: {{ t: 40, b: 50, l: 60, r: 30 }}
                }});
                return;
            ...
            const histTrace = {{
                x: state.stabilityValues, type: 'histogram',
                marker: {{ color: 'lightblue', line: {{ width: 0 }} }},
                nbinsx: 20,
                hovertemplate: 'Range: %{{x}}<br>Count: %{{y}}<extra></extra>'
            }};
            const shapes = [];
            const annotations = [];
            if (state.stabilityMedian !== null) {{
                shapes.push({{ type: 'line', x0: state.stabilityMedian, x1: state.stabilityMedian, y0: 0, y1: 1, yref: 'paper', line: {{ color: 'teal', width: 2, dash: 'dash' }} }});
                annotations.push({{ x: 0.95, y: 0.95, xref: 'paper', yref: 'paper', text: `Median = ${{state.stabilityMedian.toFixed(2)}}`, showarrow: false, font: {{ size: 11, color: 'teal' }}, xanchor: 'right', yanchor: 'top' }});
            }}
            if (state.stabilityMean !== null) {{
                shapes.push({{ type: 'line', x0: state.stabilityMean, x1: state.stabilityMean, y0: 0, y1: 1, yref: 'paper', line: {{ color: 'coral', width: 2, dash: 'dot' }} }});
                annotations.push({{ x: 0.95, y: 0.85, xref: 'paper', yref: 'paper', text: `Mean = ${{state.stabilityMean.toFixed(2)}}`, showarrow: false, font: {{ size: 11, color: 'coral' }}, xanchor: 'right', yanchor: 'top' }});
            }}
            const layout = {{
            ...
            plot('stability-hist-main', [histTrace], layout);
        }}

        function highlightStabilityValue(cellId) {{
            if (!state) return;
            const info = state.cellInfo[cellId.toString()];
            if (!info || info.odd_even_corr === null) return;
            const div = document.getElementById('stability-hist-main');
            if (!div || !div.layout) return;
            const ex = div.layout.shapes ? [...div.layout.shapes] : [];
            const filtered = ex.filter(s => s.line.color !== 'red');
            filtered.push({{
            ...
            Plotly.relayout('stability-hist-main', {{ shapes: filtered, annotations: filteredA }});
        }}

        function createSigStabilityHist() {{
            if (!state || !state.sigStabilityValues || state.sigStabilityValues.length === 0) {{
                plot('sig-stability-box-main', [], {{
            ...
            const histTrace = {{
                x: state.sigStabilityValues, type: 'histogram',
                marker: {{ color: 'lightgreen', line: {{ width: 0 }} }},
                nbinsx: 20,
                hovertemplate: 'Range: %{{x}}<br>Count: %{{y}}<extra></extra>'
            }};
            const shapes = [];
            const annotations = [];
            if (state.sigStabilityMedian !== null) {{
                shapes.push({{ type: 'line', x0: state.sigStabilityMedian, x1: state.sigStabilityMedian, y0: 0, y1: 1, yref: 'paper', line: {{ color: 'teal', width: 2, dash: 'dash' }} }});
                annotations.push({{ x: 0.95, y: 0.95, xref: 'paper', yref: 'paper', text: `Median = ${{state.sigStabilityMedian.toFixed(2)}}`, showarrow: false, font: {{ size: 11, color: 'teal' }}, xanchor: 'right', yanchor: 'top' }});
            }}
            if (state.sigStabilityMean !== null) {{
                shapes.push({{ type: 'line', x0: state.sigStabilityMean, x1: state.sigStabilityMean, y0: 0, y1: 1, yref: 'paper', line: {{ color: 'coral', width: 2, dash: 'dot' }} }});
                annotations.push({{ x: 0.95, y: 0.85, xref: 'paper', yref: 'paper', text: `Mean = ${{state.sigStabilityMean.toFixed(2)}}`, showarrow: false, font: {{ size: 11, color: 'coral' }}, xanchor: 'right', yanchor: 'top' }});
            }}
            annotations.push({{ x: 0.95, y: 0.75, xref: 'paper', yref: 'paper', text: `n = ${{state.sigStabilityValues.length}}`, showarrow: false, font: {{ size: 11, color: 'gray' }}, xanchor: 'right', yanchor: 'top' }});
            const layout = {{
            ...
            plot('sig-stability-box-main', [histTrace], layout);
        }}

        function recomputeSignificance() {{
            if (!state) return;
            const sigIds = [];
            const sigPeaks = [];
            const sigStab = [];
            for (const key in state.cellInfo) {{
                const info = state.cellInfo[key];
                if (!isCellSignificant(info)) continue;
                sigIds.push(parseInt(key));
                sigPeaks.push(info.peak_position);
            ...
            const nBins = xaxis.length;
            const heatmapRows = sortedIds.map(id => {{
                const tr = state.traces[id.toString()];
                return tr ? tr : new Array(nBins).fill(0);
            }});
            state.sequenceHeatmap = heatmapRows.length > 0 ? heatmapRows : null;
            state.sequenceCellIds = sortedIds;
            state.sequencePeakTimes = sortedPeaks;
            state.sigStabilityValues = sigStab;
            state.sigStabilityMedian = computeMedian(sigStab);
            state.sigStabilityMean = computeMean(sigStab);

            createSequenceHeatmap();
            createSigStabilityHist();
            if (selectedRoiId !== null) {{
                highlightSequenceCell(selectedRoiId);
                updateTrace(selectedRoiId);
            }}

            const nSig = sortedIds.length;
            const tot = state.totalCells;
            const pct = tot > 0 ? (100 * nSig / tot) : 0;
            document.getElementById('stats-counter-main').innerHTML =
                `${{nSig}}/${{tot}} significant (${{pct.toFixed(1)}}%)`;
        }}
        ...
        function resetTracePanel() {{
            ...
            plot('trace-plot-main', [], emptyLayout);
            const emptyPerLapLayout = {{
                title: `Per-Lap (Select a cell)`,
                autosize: true,
                xaxis: {{ title: xLabel, range: [0, maxXLimit] }},
                yaxis: {{ title: 'Trial' }},
                margin: {{ t: 40, b: 60, l: 60, r: 80 }}
            }};
            plot('per-lap-heatmap-main', [], emptyPerLapLayout);
            document.getElementById('roi-info-main').innerHTML = 'Click an ROI on the map to view its field map.';
        }}
        ...
        document.addEventListener('keydown', function(e) {{
            ...
            if (document.activeElement && document.activeElement.tagName === 'INPUT') return;
            e.preventDefault();
            if (!state || !state.sequenceCellIds || state.sequenceCellIds.length === 0) return;
            const ids = state.sequenceCellIds;
            let curIdx = -1;
            if (selectedRoiId !== null) {{
                curIdx = ids.indexOf(parseInt(selectedRoiId));
            }}
