...
#%% skip registraion and load bin file directly
...
for rec in rec_lst:
        ...
        p_suite2p = p_data/'suite2p'/'plane0'  
        f_bin_ch1 = p_suite2p/'data.bin'
        f_bin_ch2 = p_suite2p/'data_chan2.bin'
        data = np.memmap(f_bin_ch1, dtype='int16', mode='r', shape=(1000, 512, 512))
        ...
        save_path = p_out
        settings['diameter'] = [5, 5] # Expected cell diameter in pixels, passed to Cellpose, by default [12.0, 12.0]
        settings['detection']['cellpose_settings']['img'] = 'meanImg' # Which image to segment: 'max_proj / meanImg', 'meanImg', or 'max_proj'
        settings['detection']['cellpose_settings']['flow_threhold'] = 1 # default 0.4
        pipeline(save_path, f_reg=data, run_registration=False, settings=settings, device=torch.device("cpu"))
