...
settings['detection']['cellpose_settings']['flow_threshold'] = 3 # default 0.4
settings['detection']['params'] = {'niter': 400, }
#%% 
...
for rec in rec_lst:
        ...
        if run_detection_only: 
            ...
            ops = np.load(p_suite2p/'ops.npy', allow_pickle=True).item()
            f_bin_ch1 = p_suite2p/'data.bin'
            f_bin_ch2 = p_suite2p/'data_chan2.bin'
            nframes = ops['nframes']
            # nframes = 5000
            data = np.memmap(f_bin_ch1, dtype='int16', mode='r', shape=(nframes, 512, 512))
            ...
            shutil.copy2(p_suite2p/'ops.npy', p_out/'ops.npy')  # copies file + metadata
            # shutil.copy(src, dst)   # copies file only
